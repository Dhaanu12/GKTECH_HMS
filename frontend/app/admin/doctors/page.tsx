...
import { Plus, Building2, Loader2, Edit2, MapPin, Power, Search, CreditCard, Stethoscope } from 'lucide-react';
...
export default function BranchesPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            const token = localStorage.getItem('token');
            let formDataPayload: any = { ...formData, enabled_modules: JSON.stringify(formData.enabled_modules) };
            if (editingBranch) {
                await axios.put(`${API_URL}/branches/${editingBranch.branch_id}`, formDataPayload, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            } else {
                formDataPayload = { ...formData, enabled_modules: JSON.stringify(formData.enabled_modules) };
                await axios.post(`${API_URL}/branches`, formDataPayload, {
                    headers: { Authorization: `Bearer ${token}` }
                });
            }
            setShowModal(false);
    ...
    const handleEdit = async (branch: any) => {
        ...
        setFormData({
            ...
            department_ids: [],
            service_ids: [],
            enabled_modules: Array.isArray(branch.enabled_modules)
                ? branch.enabled_modules.map((m: any) => typeof m === 'string' ? { id: m, is_active: true } : m)
                : []
        });
        setShowModal(true);
        ...
        try {
            ...
            const deptIds = deptRes.data.data.departments.map((d: any) => d.department_id);
            const svcIds = svcRes.data.data.services.map((s: any) => s.service_id);
    ...
    const hospitalOptions = hospitals.map((hospital: any) => ({
        value: String(hospital.hospital_id),
        ...
        <div>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 shadow-sm mb-6">
                <div className="relative">
                    ...
                    <input
                        type="text"
                        placeholder="Search branches by name, code, or hospital..."
                        value={searchTerm}
                        ...
                        <div key={branch.branch_id} className={`bg-white border rounded-lg p-6 hover:shadow-lg transition group ${!branch.is_active ? 'opacity-60 border-gray-300' : 'border-gray-200'}`}>
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <button
                                        ...
                                        className={`transition ${branch.is_active ? 'text-green-600 hover:text-green-700' : 'text-gray-400 hover:text-gray-500'}`}
                            ...
                            <div className="mb-3">
                                <span className="inline-block px-3 py-1 bg-blue-50 text-blue-700 text-xs font-medium rounded-full border border-blue-200">
                                    {branch.hospital_name || hospitals.find((h: any) => h.hospital_id === branch.hospital_id)?.hospital_name || 'Unknown Hospital'}
                                </span>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-white rounded-2xl max-w-2xl w-full shadow-2xl overflow-hidden">
                        <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                            ...
                            <button onClick={() => setViewingBranch(null)} className="text-gray-400 hover:text-gray-600">
                        ...
                        <div className="p-6 space-y-4">
                            <div className="flex items-center gap-4 mb-6">
                                ...
                                <div>
                                    <h3 className="text-2xl font-bold text-gray-900">{viewingBranch.branch_name}</h3>
                                    <p className="text-blue-600 font-medium">{viewingBranch.branch_code}</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <p className="text-gray-900 font-medium">
                                        {viewingBranch.hospital_name || hospitals.find((h: any) => h.hospital_id === viewingBranch.hospital_id)?.hospital_name || 'N/A'}
                                    </p>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 uppercase">Contact Number</label>
                                    <p className="text-gray-900 font-medium">{viewingBranch.contact_number || 'N/A'}</p>
                                </div>
                                <div className="col-span-2">
                                    <label className="text-xs font-semibold text-gray-500 uppercase">Address</label>
                                    <p className="text-gray-900 font-medium">{viewingBranch.address_line1}</p>
                                    <p className="text-gray-900 font-medium">{viewingBranch.city}, {viewingBranch.state} - {viewingBranch.pincode}</p>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 uppercase">Emergency Services</label>
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${viewingBranch.emergency_available ? 'bg-red-100 text-red-800' : 'bg-gray-100 text-gray-800'}`}>
                                        {viewingBranch.emergency_available ? 'Available' : 'Not Available'}
                                    </span>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 uppercase">MLC Fee</label>
                                    <p className="text-gray-900 font-medium">₹{viewingBranch.mlc_fee || '0.00'}</p>
                                </div>
                                <div>
                                    <label className="text-xs font-semibold text-gray-500 uppercase">Status</label>
                                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${viewingBranch.is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                                        {viewingBranch.is_active ? 'Active' : 'Inactive'}
                                    </span>
                                </div>
                                <div className="col-span-2">
                                    <label className="text-xs font-semibold text-gray-500 uppercase">Enabled Modules</label>
                                    <div className="flex flex-wrap gap-2 mt-2">
                                        {viewingBranch.enabled_modules && viewingBranch.enabled_modules.length > 0 ? (
                                            viewingBranch.enabled_modules.map((mod: any) => {
                                            ...
                                            <span className="text-gray-500 italic">No modules enabled (Inherits all or None?)</span>
                                        )}
                                    </div>
                                </div>
                            </div>
                            ...
                            <form onSubmit={handleSubmit} className="p-6 space-y-6">
                                ...
                                {(formData.hospital_id || user?.hospital_id) && (
                                    <div className="pt-6 border-t border-gray-200/50">
                                        <h3 className="text-lg font-semibold text-gray-900 mb-3">Module Access</h3>
                                        <p className="text-sm text-gray-500 mb-4">Configure which modules are available for this branch. Only modules enabled for the Hospital can be assigned.</p>
                                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                                            {(() => {
                                                const selectedHospitalId = formData.hospital_id || user?.hospital_id;
                                                const hospital = hospitals.find(h => String(h.hospital_id) === String(selectedHospitalId));

                                                // Get Hospital's allowed modules (Set of IDs)
                                                // Handling both legacy string[] and new object[] structure in hospital data
                                                const hospitalModules = new Set<string>();
                                                if (hospital && hospital.enabled_modules) {
                                                    if (Array.isArray(hospital.enabled_modules)) {
                                                        hospital.enabled_modules.forEach((m: any) => {
                                                            const id = typeof m === 'string' ? m : m.id;
                                                            const isActive = typeof m === 'string' ? true : m.is_active;
                                                            if (isActive) hospitalModules.add(id);
                                                        });
                                                    }
                                                } else if (hospital && hospital.enabled_modules === null) {
                                                    // If null, assuming defaults? Or none? 
                                                    // Migration replaced nulls with defaults.
                                                }

                                                return ACCESS_MODULES.map((module) => {
                                                    // Check if Hospital allows this module
                                                    const isAllowedByHospital = hospitalModules.has(module.id);
                                                    if (!isAllowedByHospital) return null; // Skip if hospital doesn't have it
