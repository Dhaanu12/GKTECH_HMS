...
export default function ClientAdminReports() {
    ...
    const fetchData = useCallback(async () => {
        console.log('🔄 fetchData called with:', { activeTab, dateRange });
        setLoading(true);
        setData(null);
        setError(null);
        ...
        try {
            ...
            console.log('📤 API Request:', { activeTab, params });
    ...
    const handleDateChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        console.log('📅 Date changed:', e.target.name, e.target.value);
        setDateRange(prev => {
            const newRange = { ...prev, [e.target.name]: e.target.value };
    ...
    const handleApplyFilter = () => {
        console.log('🔍 Apply filter clicked - forcing refetch');
        fetchData();
    };
    ...
    console.log('🎨 Render - Current state:', { activeTab, dateRange, loading, hasData: !!data, error });
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700 min-h-screen pb-20">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 text-sm">
                <strong className="text-blue-900">Debug Panel:</strong>
                <div className="grid grid-cols-2 gap-2 mt-2 text-blue-800">
                    ...
                    <div>Start Date: <strong>{dateRange.startDate}</strong></div>
                    <div>End Date: <strong>{dateRange.endDate}</strong></div>
                    <div>Has Data: <strong>{data ? 'Yes' : 'No'}</strong></div>
            ...
            <div className="min-h-[400px]">
                ...
                {loading ? (
                    <div className="flex flex-col items-center justify-center h-64 gap-4">
                        ...
                        <p className="text-slate-500 text-sm">Loading reports for {dateRange.startDate} to {dateRange.endDate}...</p>
                    </div>
                ) : (
                    <>
