        ...
        <div className="max-w-5xl mx-auto space-y-4 -mt-4">
            ...
            {activeTab === 'consultation' ? (
                <>
                    ...
                    <div className="bg-gradient-to-r from-blue-600 to-indigo-700 py-5 px-8 rounded-3xl border border-white/10 relative overflow-hidden shadow-2xl shadow-blue-900/20">
                        <div className="absolute top-0 right-0 w-[400px] h-[400px] bg-gradient-to-bl from-white/10 to-transparent rounded-full blur-3xl -mr-20 -mt-20 pointer-events-none"></div>

                        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
                            <div className="flex items-center gap-5">
                                <div className="w-16 h-16 bg-white/10 rounded-2xl flex items-center justify-center text-white text-2xl font-bold backdrop-blur-md border border-white/20 shadow-inner ring-2 ring-white/30">
                                    {patient?.first_name?.charAt(0)}
                                </div>
                                <div>
                                    <div className="flex items-center gap-3">
                                        <h1 className="text-2xl font-heading font-black text-white tracking-tight">{patient?.first_name} {patient?.last_name}</h1>
                                        {opdHistory.some((opd: any) => opd.is_mlc) && (
                                            <span className="px-3 py-1 bg-rose-500 text-white border border-rose-400 text-xs font-bold rounded-full uppercase tracking-wider flex items-center gap-1.5 shadow-lg shadow-rose-900/20 animate-pulse">
                                                <Activity className="w-3.5 h-3.5 fill-current" /> MLC
                                            </span>
                                        )}
                                    </div>
                                    <div className="flex flex-wrap items-center gap-2.5 text-blue-50 text-[11px] mt-2 font-medium">
                                        <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-xl backdrop-blur-md border border-white/10 shadow-sm"><User className="w-3.5 h-3.5 text-blue-200" /> {patient?.age} Yrs • {patient?.gender}</span>
                                        <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-xl backdrop-blur-md border border-white/10 shadow-sm"><FileText className="w-3.5 h-3.5 text-blue-200" /> MRN {patient?.mrn_number}</span>
                                        <span className="flex items-center gap-1.5 bg-white/10 px-3 py-1.5 rounded-xl backdrop-blur-md border border-white/10 shadow-sm"><Phone className="w-3.5 h-3.5 text-blue-200" /> {patient?.contact_number}</span>
                                    </div>
                                </div>
                            </div>
                                    ...
                                    if (activeVisit && !showConsultationForm) {
                                        ...
                                        return (
                                            <button
                                                ...
                                                className="px-6 py-3 bg-white hover:bg-slate-50 text-blue-600 rounded-xl transition-all font-black flex items-center gap-2 shadow-xl shadow-white/20 hover:scale-105 active:scale-95 group text-sm"
                                            >
                                    ...
                                    if (completedVisit && !activeVisit && !showConsultationForm) {
                                        return (
                                            <button
                                                ...
                                                className="px-6 py-3 bg-white/20 hover:bg-white/30 text-white border border-white/30 rounded-xl transition-all font-black flex items-center gap-2 shadow-xl hover:scale-105 active:scale-95 group text-sm backdrop-blur-md"
                                            >
