...
export default function ReportsPage() {
    ...
    // Process data for charts with proper calculations
    const doctorData = (data?.doctorStats || []).map(doc => {
        const patientCount = parseInt(doc.patient_count);
        const revenue = parseFloat(doc.revenue_generated);
        return {
            name: `Dr. ${doc.first_name} ${doc.last_name}`,
            patients: patientCount,
            revenue: revenue,
            avgRevenuePerPatient: patientCount > 0 ? revenue / patientCount : 0,
    ...
    return (
        <div className="space-y-8 animate-in fade-in duration-700">
            ...
            <div className="flex flex-col md:flex-row justify-between items-end gap-6">
                ...
                <div className="flex items-center gap-4 bg-white/60 p-2 rounded-xl border border-white/60 shadow-sm backdrop-blur-sm">
                    ...
                    <input
                        ...
                        className="bg-white/80 border-none rounded-lg text-slate-700 font-semibold focus:ring-2 focus:ring-blue-500/20 shadow-inner px-3 py-1.5 text-sm"
                    />
                    <button onClick={fetchAnalytics} className="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition" title="Refresh">
                        <Filter className="w-4 h-4" />
                    </button>
                </div>
            </div>
            ...
            {/* Doctor Performance - Separated Charts */}
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={doctorData} layout="vertical" margin={{ left: 20, right: 20 }}>
                                    ...
                                    <Tooltip
                                        ...
                                        contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.95)', backdropFilter: 'blur(10px)', borderRadius: '12px', border: 'none', boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}
                                    />
                                    <Legend />
                                    <Bar dataKey="patients" name="Patients Handled" fill="#3b82f6" radius={[0, 8, 8, 0]} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                    ...
                    {/* Average Revenue Per Patient - More Meaningful Metric */}
                    <div className="glass-panel p-6 rounded-3xl border border-white/60 lg:col-span-2">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-lg font-bold text-slate-800 flex items-center gap-2">
                            ...
                            <p className="text-xs text-slate-500">Higher values indicate higher-value consultations or procedures</p>
                        </div>
                        <div className="h-80 w-full">
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={doctorData} margin={{ top: 20, right: 30, left: 20, bottom: 5 }}>
                                    ...
                                    />
                                    <Legend />
                                    <Bar dataKey="avgRevenuePerPatient" name="Avg Revenue per Patient (₹)" fill="#8b5cf6" radius={[8, 8, 0, 0]} barSize={60} />
                                </BarChart>
                            </ResponsiveContainer>
                        </div>
                    </div>
                ...
                <div className="glass-panel rounded-3xl border border-white/60 overflow-hidden">
                    <div className="p-6 border-b border-gray-100">
                        <h3 className="text-lg font-bold text-slate-800">Detailed Performance Metrics</h3>
                    </div>
                        ...
                        <table className="w-full text-left text-sm">
                            <thead className="bg-slate-50 text-xs uppercase font-semibold text-slate-600">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 text-right">Patients</th>
                                    <th className="px-6 py-4 text-right">Total Revenue</th>
                                    <th className="px-6 py-4 text-right">Avg/Patient</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {doctorData.map((doc, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/50 transition">
                                        ...
                                        <td className="px-6 py-4 text-right font-medium text-slate-700">{doc.patients}</td>
                                        <td className="px-6 py-4 text-right font-bold text-emerald-600">
                                            ₹{doc.revenue.toLocaleString()}
                                        </td>
                                        <td className="px-6 py-4 text-right font-semibold text-purple-600">
                                            ₹{doc.avgRevenuePerPatient.toFixed(2)}
                                        </td>
                                            ...
                                            <div className="flex items-center justify-center gap-1">
                                                <div className="w-20 h-2 bg-gray-200 rounded-full overflow-hidden">
                                                    <div
                                                        ...
                                                        style={{
                                                            width: `${Math.min((doc.avgRevenuePerPatient / Math.max(...doctorData.map(d => d.avgRevenuePerPatient))) * 100, 100)}%`
                                                        }}
                                                    />
                                                </div>
                                            </div>
