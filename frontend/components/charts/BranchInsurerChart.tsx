...
export default function BranchInsurerChart() {
    const [branches, setBranches] = useState<any[]>([]);
    const [selectedBranchId, setSelectedBranchId] = useState<string>('');
    const [data, setData] = useState<any>({ branch: {}, insurers: [], summary: {} });
    ...
    const fetchBranches = async () => {
        try {
            ...
            const response = await axios.get(`${API_URL}/accountant/assigned-branches`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            const fetchedBranches = response.data.data.branches || [];
            setBranches(fetchedBranches);
            if (fetchedBranches.length > 0) {
                setSelectedBranchId(fetchedBranches[0].branch_id.toString());
            }
        } catch (error) {
    ...
    const fetchBranchData = async (branchId: string) => {
        try {
            setLoading(true);
            const token = localStorage.getItem('token');
            const response = await axios.get(`${API_URL}/accountant/analytics/branch-insurers/${branchId}`, {
                headers: { Authorization: `Bearer ${token}` }
            });
            setData(response.data.data);
        } catch (error) {
            console.error('Error fetching branch insurer analytics:', error);
        } finally {
            setLoading(false);
        }
    };
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b border-gray-200 bg-gray-50">
                                            ...
                                            <th className="text-right py-3 px-4 font-medium text-gray-700">Avg Bill</th>
                                    ...
                                    <tbody>
                                        {data.insurers.map((insurer: any, idx: number) => (
                                            <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50">
                                                ...
                                                <td className="py-3 px-4 text-right text-blue-600">{insurer.claims_percentage}%</td>
                                                <td className="py-3 px-4 text-right">₹{insurer.total_bill_amount?.toLocaleString()}</td>
                                                <td className="py-3 px-4 text-right">₹{insurer.avg_bill_amount?.toLocaleString()}</td>
