...
export default function MarketingManagement() {
    ...
    const canManageMarketing = user?.role_code === 'SUPER_ADMIN' || hasAccess('market');
        ...
        <div>
            <div className="flex justify-between items-center mb-6">
                ...
                <button
                    ...
                    title={canManageMarketing ? '' : 'Marketing module is not enabled for your hospital'}
                    className={`flex items-center gap-2 px-4 py-2.5 text-white rounded-lg transition font-medium shadow-lg ${
                        canManageMarketing
                            ? (activeTab === 'managers'
                                ? 'bg-blue-600 hover:bg-blue-700 shadow-blue-500/30'
                                : 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-500/30')
                            : 'bg-indigo-600 shadow-indigo-500/30 opacity-40 cursor-not-allowed blur-[0.5px]'
                    }`}
                >
                    ...
                    {marketingUsers.map((u: any) => (
                        <div key={u.user_id} className="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-lg transition group">
                            <div className="flex items-start justify-between mb-4">
                                ...
                                <button
                                    onClick={() => { if (canManageMarketing) handleEdit(u); }}
                                    disabled={!canManageMarketing}
                                    title={canManageMarketing ? 'Edit Profile' : 'Marketing module is not enabled'}
