...
CREATE TABLE IF NOT EXISTS referral_payment_upload_batch (
...
CREATE TABLE IF NOT EXISTS referral_payment_header (
    id SERIAL PRIMARY KEY,
    uuid VARCHAR(50) UNIQUE NOT NULL DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS referral_payment_details (
    id SERIAL PRIMARY KEY,
    uuid VARCHAR(50) UNIQUE NOT NULL DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_batch_hospital ON referral_payment_upload_batch(hospital_id);
CREATE INDEX IF NOT EXISTS idx_header_batch ON referral_payment_header(batch_id);
CREATE INDEX IF NOT EXISTS idx_details_header ON referral_payment_details(payment_header_id);
